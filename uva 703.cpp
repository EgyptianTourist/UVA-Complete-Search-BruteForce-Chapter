    ...
    while(scanf("%d", &n)==1)
    {
        ...
        rep(i, 1, n)
        {
            rep(j, 1, n)
            {
                rep(k, 1, n)
                {
                    if(((i < j && j < k) || (i > j && j > k)) && game[i][j] && game[j][k] && game[k][i])
                    {
                        vector<int> tmp;
                        tmp.pb(i);
                        tmp.pb(j);
                    ...
                    else if ((i < j && j < k) && game[i][j] + game[j][i] + game[j][k] + game[k][j] + game[k][i] + game[i][k] == 0)
                    {
                        vector<int> tmp;
                        tmp.pb(i);
                        tmp.pb(j);
